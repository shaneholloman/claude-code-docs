...
if [ -f "docs/docs_manifest.json" ]; then
    ...
    if ! git clone https://github.com/shaneholloman/claude-code-docs.git; then
...
Error handling:
- If any files are missing or commands fail, show: "Error accessing docs. Try re-running: curl -fsSL https://raw.githubusercontent.com/shaneholloman/claude-code-docs/main/install.sh | bash"
...
echo ""
echo "Note: Restart Claude Code for auto-updates to take effect"
echo ""
